...
export namespace ArrayOrObject {
  ...
  export function collectionPropertyInUri(value: Utils.SourceArray, index: number): Lexer.Token {
    ...
    let prop = NameOrIdentifier.primitiveColProperty(value, index) || NameOrIdentifier.complexColProperty(value, index);
  ...
  export function charInJSON(value: Utils.SourceArray, index: number): number {
    ...
    if (escape > index) {
      ...
      if (
        ...
        return escape + 1;
      if (Utils.equals(value, escape, "u") && Utils.required(value, escape + 1, Lexer.HEXDIG, 4, 4)) return escape + 5;
  ...
  export function numberInJSON(value: Utils.SourceArray, index: number): Lexer.Token {
    let token = PrimitiveLiteral.doubleValue(value, index) || PrimitiveLiteral.int64Value(value, index);
    if (token) {
      token.value = "number";
      return token;
    }
