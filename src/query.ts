...
export namespace Query {
  ...
  export function searchExpr(value: Utils.SourceArray, index: number): Lexer.Token {
    let token = Query.searchParenExpr(value, index) || Query.searchTerm(value, index);

    if (!token) return;
    let start = index;
    index = token.next;

    let expr = Query.searchAndExpr(value, index) || Query.searchOrExpr(value, index);
  ...
  export function searchNotExpr(value: Utils.SourceArray, index: number): Lexer.Token {
    ...
    if (rws === index) return;
    index = rws;
    let expr = Query.searchPhrase(value, index) || Query.searchWord(value, index);
    if (!expr) return;
    index = expr.next;
  ...
  export function selectItem(value: Utils.SourceArray, index: number): Lexer.Token {
    ...
    if (op > index) {
      ...
      item = {};
      let name = NameOrIdentifier.qualifiedEntityTypeName(value, index) || NameOrIdentifier.qualifiedComplexTypeName(value, index);
      ...
      let select = Query.selectProperty(value, index) || Query.qualifiedActionName(value, index) || Query.qualifiedFunctionName(value, index);
      if (!select) return;
      index = select.next;
  ...
  export function selectPath(value: Utils.SourceArray, index: number): Lexer.Token {
    let token = NameOrIdentifier.complexProperty(value, index) || NameOrIdentifier.complexColProperty(value, index);
